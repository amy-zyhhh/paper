...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch journal article metadata from Crossref and merge into data/papers.json."
    )
    parser.add_argument("--issn", required=True, help="Journal ISSN, for example 0022-5096.")
    parser.add_argument("--journal", default="", help="Journal display name.")
    parser.add_argument("--limit", type=int, default=25, help="Maximum records to request.")
    parser.add_argument("--days", type=int, default=365, help="Fetch papers from the last N days.")
    parser.add_argument("--from-date", default="", help="Start publication date, YYYY-MM-DD.")
    parser.add_argument("--until-date", default="", help="End publication date, YYYY-MM-DD.")
...
def request_json(url: str) -> dict[str, Any]:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "literature-tracker/0.2",
        },
    )
...
def normalize_item(item: dict[str, Any], journal_name: str, issn: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "doi": doi,
        "date": paper_date(item),
        "journal": journal_name or container_title or issn,
        "container_title": container_title,
        "issn": item.get("ISSN", [issn]),
...
def build_crossref_url(args: argparse.Namespace) -> str:
    from_date = validate_date(args.from_date, "--from-date")
    until_date = validate_date(args.until_date, "--until-date")
    if not from_date:
    ...
    issn = urllib.parse.quote(args.issn)
...
def main() -> int:
    args = parse_args()
    try:
        payload = request_json(build_crossref_url(args))
